...
beforeAll(async () => {
  ...
  const record = await (await config.metaNamesContract.domainRepository.find(domainName))?.recordRepository.find(recordClass)
...
test('lookup domain record with non existent record', async () => {
  const data = await (await config.metaNamesContract.domainRepository.find(domainName))?.recordRepository.find(RecordClassEnum.Twitter)
