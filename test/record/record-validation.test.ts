...
describe('Record Validator Factory', () => {
  test('returns DefaultRecordValidator for Bio record', () => {
    ...
    const validator = getRecordValidator(record.class)
  ...
  test('throws error for invalid record class', () => {
    ...
    expect(() => getRecordValidator(record.class)).toThrow('Record class is invalid')
  })
  ...
  test('caches and returns the same validator instance for the same record class', () => {
    ...
    const validator1 = getRecordValidator(record1.class)
    const validator2 = getRecordValidator(record2.class)
  ...
  test('returns different validator instances for different record classes', () => {
    ...
    const bioValidator = getRecordValidator(bioRecord.class)
    const emailValidator = getRecordValidator(emailRecord.class)
